...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    for ($i = 1; $i <= $jumlah_soal; $i++) {
        ...
        $benar == 1 ? $skor += $skorPerSoal : '';

        // mysqli_query($connect, "INSERT INTO jawaban_mhs (id_jawaban, id_detail_soal, id_detail_room, jawaban, benar) VALUES ('$id_jawaban', '$id_detail_soal', '$id_detail_room', '$jawaban', '$benar')");

        // echo $benar . '<br>' . $jawaban . '<br>' . $dataSoal['jawaban'] . '<br>' . $dataSoal['id_soal'] . '<br>';
    }

    // mysqli_query($connect, "INSERT INTO skor_level (id_detail_level, id_detail_room, skor_mhs) VALUES ('$id_detail_level', '$id_detail_room', '$skor')");
}
